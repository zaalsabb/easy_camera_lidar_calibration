...
class VisOpen3D:
    ...
    def update_view_point(
        ...
        ctr.set_constant_z_near(znear)
        ctr.set_constant_z_far(zfar)
        param = self.convert_to_open3d_param(intrinsic, extrinsic)
        ctr.convert_from_pinhole_camera_parameters(param, allow_arbitrary=True)
        self.__vis.get_render_option().point_size = point_size
        self.__vis.get_render_option().background_color = np.asarray(background_color)
        self.__vis.update_renderer()
...
def project_2d_to_3d(pts_2d, D, K, T_c_m, w, h):
    ...
    z_c = np.array(d, dtype=np.float64) / 1000
    x_c = z_c * (u - cx) / fx
    y_c = z_c * (v - cy) / fy
...
def cloud_to_depth(pcd, K, T_c_m, w, h, point_size=2, s=1):
